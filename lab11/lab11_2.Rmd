...
q <- homerange %>%
  ...
  ggplot(aes(x=log10_mass, y=log10_preymass, color=thermoregulation))+geom_point()+geom_smooth(method="lm", se=F, color="red")
```
...
```{r}
p+theme_linedraw()+
  theme(legend.position = "bottom", #adjust the position of legend
...
```{r}
q+theme_classic()+scale_colour_brewer(palette = "Pastel1")
```
...
```{r}
my_palette <- paletteer_d("feathers::oriole") #package::palette
my_palette
```
...
```{r}
q+scale_fill_manual(values=my_cols)
```
...
homerange %>% 
  ggplot(aes(x=taxon, y=log10_mass, fill=thermoregulation))+
  geom_boxplot()+
  facet_grid(trophic_guid~thermoregulation~., labeller=label_both)+ 
  theme(axis.text.x=element_text(angel=60, hjust=1),
        legent.position="none",
        strip_text=element_text(size=12, face="bold"))
```
