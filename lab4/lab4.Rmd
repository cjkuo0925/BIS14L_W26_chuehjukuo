...
```{r}
ggplot(data=penguins, mapping=aes(x=body_mass_g, y=flipper_length_mm))+ geom_point(na.em=T)
```

To add a regression (best of fit) line, we add another layer.
```{r}
ggplot(data=penguins, mapping=aes(x=body_mass_g, y=flipper_length_mm))+ geom_point(na.em=T)+geom_smooth(method=lm, se=T)
```
...
```{r}
ggplot(data=penguins, mapping=aes(x=body_mass_g, y=flipper_length_mm, color=species))+ geom_point(na.em=T)+geom_smooth(method=lm, se=T)
```
...
```{r}
ggplot(data=penguins, mapping=aes(x=body_mass_g, y=flipper_length_mm))+ geom_point(mapping=aes(color=species, shape=species))+geom_smooth(method=lm, se=T)

```

But don't we want a title? We do this using the `labs()` function.
```{r}
ggplot(data=penguins, mapping=aes(x=body_mass_g, y=flipper_length_mm))+ geom_point(mapping=aes(color=species, shape=species))+geom_smooth(method=lm, se=T)+labs(title="Body Mass(g) vs. Flipper Length(mm)", x="Body mass(g)")
...
```{r}
ggplot(data=penguins, mapping=aes(x=bill_length_mm, y=bill_depth_mm))+geom_point(mapping=aes(color=species,shae=species))+geom_smooth(method=lm,se=F)+labs(title="Bil Length(mm) vs. Bill Depth(mm)")
```
...
names(penguins)
```
How many individuals per secies?
```{r}
...
```{r}
ggplot(data=penguins, mapping=aes(x=species))+geom_bar(mapping=aes(color=species))
```
...
```{r}
ggplot(data=penguins, mapping=aes(x=species))+geom_bar(mapping=aes(fill=species))
...
```{r}
ggplot(data=penguins, mapping=aes(x=island))+geom_bar(mapping=aes(fill=island))
```
...
```{r}
ggplot(data=penguins, mapping=aes(x=island))+geom_bar(mapping=aes(fill=species), position="dodge")+labs(title="#Penguins for each speies by island")
```
