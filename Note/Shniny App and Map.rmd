...
#bounding box — the rectangular geographic area that contains all your data points
#f act as buffer
```
...
2. Shiny app with two variables
```{r}
ui <- fluidPage(
  selectInput("x", 
              "Select order",
...
3. Shiny app with kind of web pages(with sidebar on)
```{r}
ui <- dashboardPage(
  
  dashboardHeader(title="Distribution ofGestation Length by Order"),
...
4.Shiny app with more complicated setting
```{r}
ui <- dashboardPage(
  ...
  selectInput("x", 
              "Select population",
              choices= sort(unique(wolves$pop)),#don't need totype every popuation
              selected="AK.PEN")),
...
server <- function(input, output, session) {
  output$plot <- renderPlot({
    wolves %>% 
      ...
      geom_bar(alphas=0.8, color="black")
