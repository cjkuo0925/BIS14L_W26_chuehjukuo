...
```{r}
filter(penguins, body_mass_g %in% c(5250, 5300, 5500, 5350, 5400, 5450, 5500))
```

You can also extract observations "near" a certain value but you need to specify a tolerance. (5350-5450 for this case)
```{r}
...
penguins%>%
  select(species, body_mass_g) %>%
  filter(species=="Gentoo") %>%
  slice_max(body_mass_g, n=1) #slice_tail, slice_head
```
...
```{r}
fish %>%
  select(length, fish_id, radii_length_mm, scalelength) %>%
  filter(fish_id == 300) %>%
  arrange(length)
```
