...
gap %>%
  ...
  geom_line()+geom_point(shape=5)+
  scale_x_continuous(breaks=2000:2025, minor_breaks = NULL)+theme(axis.text.x=element_text(angle=60, hjust=1))+labs(title="GDP per Capita in the USA, 2000-2025", x="Year", y="GDP")
```

```{r}
gap %>%filter(year>=2000 & year <= 2025 & country=="USA") %>%
  ggplot(aes(x=year, y=gdp))+
  geom_line(color="red")+
  geom_point(shape=5)+
...
gap%>%
  ...
  labs(title="Distribution of Lide Expetancy, 1930 ad 1950",
...
```{r}
income_cat2000%>%
  drop_na(income_caegory) %>% #get rid of NA
  ggplot(aes(x=income_category, fill=income_caegory))+geom_bar() #lab title
```
