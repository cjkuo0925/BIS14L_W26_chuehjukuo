...
The data is not tidy, since Region are the same throughout the data and the names are not clean. This is in wider format. 
```{r}
sydneybeaches%>%
  distinct(Region)
```
...
sydneybeaches_long <- sydneybeaches %>%
  select(date, enterococci_cfu_100ml, site)
```
...
```{r}
sydneybeaches_long %>%
  select(year, enterococci_cfu_100ml, site) %>%
  group_by(year,site) %>%
  summarize(
    ave_enterococci=mean(enterococci_cfu_100ml, 
                                   na.rm = TRUE))
```
...
Little Bay Beach was the most polluted beach in 2013 since it have the highest average enterococci_cfu_100ml.
```{r}
sydneybeaches_long %>%
  filter(year=="2013")%>%
  pivot_longer(-year,
               names_to = "beach", 
              values_to = "ave_enterococci") %>%
  arrange(desc(ave_enterococci))
...
How is the enterococci_cfu_100ml of Little Bay Beach changed over sample period?(since it has the highest amounts of ecoli in 2013)
```{r}
sydneybeaches_long%>%
  pivot_longer(-year,
               names_to = "beach", 
              values_to = "ave_enterococci") %>%
  filter(beach=="Little Bay Beach")%>%
  ggplot(aes(x=as.integer(year), y=ave_enterococci))+geom_point()+
  geom_line() +labs(title="Amount of Enterococci changed from 2013-2018", x="year", y="ave_enterococci")
