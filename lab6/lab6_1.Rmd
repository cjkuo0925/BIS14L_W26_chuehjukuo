...
ggplot(data=penguins, mapping=aes(x=flipper_length_mm, y=body_mass_g))+geom_point()+geom_smooth(method="lm", se=F)
```
...
```{r}
ggplot(data=mammals, mapping=aes(x=gestation, y=log10(newborn)))+geom_point()+geom_smooth(method="lm", se=F)
```
...
```{r}
mammals %>%
  ggplot(mapping=aes(x=log10(mass), y=max_life))+geom_point()+geom_smooth(method="lm", se=F)
```
