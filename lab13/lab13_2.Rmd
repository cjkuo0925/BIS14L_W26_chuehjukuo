...
```{r, message = FALSE}
spider_location <- read_csv("data/spiders_locations.csv")%>% clean_names()
```

2. Join the files using an appropriate key and join type.
```{r}
spider_with_loc<- left_join(spider_record, spider_location, by="accession")
```

```{r}
names(spider_with_loc)
```

3. Convert the date column to a date format using lubridate.
```{r}
spider_with_loc <- spider_with_loc%>%
  mutate(date=dmy(date))
```

4. Save your final data to a `.csv`.
```{r}
write.csv(spiders_with_csv)
```
