...
```{r}
namemout <- c("Mount Everest", "K2", "Kangchenjunga", "Lhotse", "Makalu")
fth <- c(29032, 28251, 28169, 27940, 27838)
meterh <- c(8849, 8611, 8586, 8516, 8485)
```

**2. Combine these vectors into a data frame called `mountains`.**
```{r}
moutains <- tibble(namemout, fth, meterh)
```
...
```{r}
moutains <- tibble(namemout, fth, meterh, first_climbed)
```
...
```{r}
summits <- c(12800, 964, 532, 933, 450)
moutains <- tibble(namemout, fth, meterh, first_climbed, summits)
...
```{r}
fatalities <- c( 344, 96, 52, 20, 50)
moutains <- tibble(namemout, fth, meterh, first_climbed, summits, fatalities)
```
...
moutains <- tibble(namemout, fth, meterh, first_climbed, summits, fatalities, fatality_rate)
```

**8. Write your data frame to a .csv file called `mountains_data`.**
```{r}
write.csv(moutains, "mountains_data.csv", row.names=FALSE)
```
...
```{r}
mountains <- read.csv("C:/Users/jenny/OneDrive/桌面/BIS14L_W26_chuehjukuo/lab3/mountains_data.csv")
```
