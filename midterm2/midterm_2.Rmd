...
polar_bears%>%
  group_by(year)%>%
  count(popn)%>%
  ggplot(aes(x=as.factor(year), y=n, color=popn))+
  geom_point()+
  geom_line()+
  labs(title="Sampling of polar bear population by year", x="year", y="number of population")+
...
```{r}
polar_bears<- polar_bears %>%
  mutate(fasting=case_when(un_creat_ratio<=10 ~ "y",
                                   un_creat_ratio>10~ "n"))
```
...
polar_bears%>%
  ...
  theme(axis.text.x=element_text(angle=60, hjust=1))+
  labs(title="Numer of Fasting Bears for CS By Year", x="year", y="Number of Bear")
```

```{r}
polar_bears%>%
  filter(fasting=="y" & popn=="SB")%>%
  group_by(year)%>%
  count(fasting)%>%
  ggplot(aes(x=as.factor(year), y=n))+
  geom_point()+
  geom_smooth(method=lm, se=T)+
  theme(axis.text.x=element_text(angle=60, hjust=1))+
  labs(title="Numer of Fasting Bears for SB By Year", x="year", y="Number of Bear")
```
...
```{r}
ggmap(map1)+ 
  geom_point(data=polar_bears, aes(long, lat), size=0.75, color="black")+
  labs(x="Longtitude", y="latitude", title="Distribution of Polar Bear")
```
...
server <- function(input, output, session) {
  output$plot <- renderPlot({
    polar_bears %>%
       filter(denning!="NA"&mating!="NA")%>%
      ggplot(aes(x=.data[[input$x]], y=un_creat_ratio, fill=popn))+
      geom_boxplot(alphas=0.8, color="black")+
      labs(title="Range of un_creat_ratio by Chosen Vairables")
