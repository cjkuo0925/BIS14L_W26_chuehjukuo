...
```{r}
x <- 42 #assign number to x/x gets the value 42
#remember to run this line first 
```
...
```{r}
z=10 #this work but create problems
```
...
```{r}
mean <- 20 #work but terrible name
```
...
```{r}
mean(c(2,8,2)) #c stand for combine and creates a vector
```
...
```{r}
meanspeed <- (venom_GT+chiron+veyron)/3
meanspeed
```
...
```{r}
is.integer(my_numeric) #important when working with plot
```
...
```{r}
my_missing <- NA #NA stands for misssing
```
...
There is a missing data in my data therefore the computer cannot calculate the mean.
