...
```{r}
 life_history %>%
  ggplot(aes(x=order))+geom_bar()+labs(title="observations for each order of mammals ")+coord_flip()
```
...
```{r}
life_history %>%
  ggplot(aes(x=gestation, y=wean_mass))+geom_point(na.rm = T)+scale_y_log10()+geom_smooth(method=lm, se=F, na.rm = T)
```
...
```{r}
life_history %>%
  ggplot(aes(x=order, y=log10(mass)))+geom_boxplot(na.rm = T)+coord_flip()
```
...
```{r}
life_history %>%
  ggplot(aes(x=mass, y=litter_size))+geom_point(na.rm=T)+scale_x_log10()+geom_smooth(method=lm, se=F, na.rm=T)
```
...
```{r}
life_history %>%
  ggplot(aes(x=mass, y=litter_size))+geom_point(na.rm=T)+scale_x_log10()+geom_smooth(method=lm, se=F, na.rm=T)+labs(tittle="Mass vs. Litter Size", x="Mass(g)", y="Litter Size")
```
