...
sharks%>%
  ...
  group_by(county)%>%
  summarize(total_incident=sum(incident_num))%>%
...
sharks%>%
  ...
  group_by(month)%>%
  summarize(total_incident_month=sum(incident_num, na.rm=T))%>%
...
sharks%>%
  ...
  group_by(month)%>%
  summarize(total_incident_month=sum(incident_num, na.rm=T))%>%
  ggplot(aes(x=as.factor(month), y=total_incident_month))+geom_col()+labs(title="Total number of incidents for each month", x="month", y="count of incident")
```
