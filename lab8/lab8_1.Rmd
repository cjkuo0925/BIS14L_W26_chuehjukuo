...
elephants %>%
  summarize(average_age=mean(estimated_age_years, na.rm=T),
            average_shoulder_height_cm=mean(shoulder_height_in_cm, na.rm=T),
            average_tusk_length=mean(tusk_length_in_cm, na.rm=T),
            average_tusk_circumference_cm=mean(tusk_circumference_in_cm, na.rm=T))
...
```{r}
elephants %>%
  group_by(sex) #teelng how many group there are
```
...
msleep %>%
  group_by(order) %>%
   summarize(mean_brainwt=mean(brainwt, na.rm=T)) #NaN means not a number
```

```{r}
msleep %>%
  select(genus, order, brainwt) %>%
